...
interface SelectInfoModalProps {
  visible: boolean;
  onClose: () => void;
...
export function SelectInfoModal({ visible, onClose, existingCardIds, onConfirm }: SelectInfoModalProps) {
  ...
  const availableCards = useMemo(() => {
    const candidates = cards.filter(c => !existingCardIds.includes(c.id));
    if (!searchQuery.trim()) return candidates;
    const q = searchQuery.toLowerCase();
    return candidates.filter(c =>
      c.title.toLowerCase().includes(q) || c.category.includes(q)
    );
  }, [cards, existingCardIds, searchQuery]);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={handleClose}>
      <View style={[s.container, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 16 }]}>
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>Add Info to Kit</Text>
        ...
        <FlatList
          data={availableCards}
          keyExtractor={c => c.id}
          contentContainerStyle={[s.list, availableCards.length === 0 && { flex: 1 }]}
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="card-outline" size={40} color={colors.mutedForeground} />
              <Text style={s.emptyText}>
                {cards.length === 0 ? 'No info cards yet' : 'All cards already added to kit'}
              </Text>
            </View>
          }
          renderItem={({ item: c }) => {
            ...
            return (
              ...
              >
                ...
                <View style={s.rowBody}>
                  <Text style={s.rowLabel} numberOfLines={1}>{c.title}</Text>
                  <Text style={s.rowSub} numberOfLines={1}>
                    {c.isSensitive ? '••••••' : c.value} • {cat.label}
                  </Text>
                </View>
